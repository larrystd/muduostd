...
std::vector<string> split(const string& str) {
  ...
  size_t start = 0;
  size_t pos = str.find('/'); // 找到第一个/
  while (pos != string::npos) {
    if (pos > start) {
      result.push_back(str.substr(start, pos-start));  // 分割得到的子字符串, push到vector中
    }
    start = pos+1;
    pos = str.find('/', start);
  }
  ...
  if (start < str.size()) {
    result.push_back(str.substr(start));  // 最后一块字符串
  }
      ...
      systemInspector_(new SystemInspector)
{
  ...
  server_.setHttpCallback(std::bind(&Inspector::onRequest, this, _1, _2));  // httpcallback在tcp的messagecallback之中

  processInspector_->registerCommands(this);
  systemInspector_->registerCommands(this);
#ifdef HAVE_TCMALLOC    // tcmalloc?
  performanceInspector_.reset(new PerformanceInspector);
  performanceInspector_->registerCommands(this);
#endif

  loop->runAfter(0, std::bind(&Inspector::start, this)); // little race condition 定时器调用&Inspector::start, 这里很精妙, 用定时器调用server.start执行连接监听，接受，处理
}
...
void Inspector::add(const string& module,
                    ...
                    const Callback& cb,
                    const string& help) // 向Inspector中添加内容
{
  MutexLockGuard lock(mutex_);
  modules_[module][command] = cb; // 内容整合成结构性的mudules和helps
...
void Inspector::onRequest(const HttpRequest& req, HttpResponse* resp) // 将会在messageCallback中调用, 用户自定义逻辑
{
  if (req.path() == "/")
  {
    string result;
    ...
    for (std::map<string, HelpList>::const_iterator helpListI = helps_.begin();
         ...
         ++helpListI)
    {
      const HelpList& list = helpListI->second; //HelpList对象按规则加入到result， 这里是print双层map helps_
      for (const auto& it : list)
      {
        result += "/";  // helpListI->first前后加个/, proc变成 /proc/
  ...
  else
  {
    ...
    else
    {
      string module = result[0];
      MutexLockGuard lock(mutex_);
      std::map<string, CommandList>::const_iterator commListI = modules_.find(module);  // 例如http://127.0.0.1:12345/proc/overview
      if (commListI != modules_.end())
      {
        string command = result[1];
        const CommandList& commList = commListI->second;
        CommandList::const_iterator it = commList.find(command);  // command
        if (it != commList.end())
        {
          ArgList args(result.begin()+2, result.end()); // 参数 result[2:]
          if (it->second)
          {
            resp->setStatusCode(HttpResponse::k200Ok);
            resp->setStatusMessage("OK");
            ...
            resp->setBody(cb(req.method(), args));  // 按需要调用回调函数
            ok = true;
          }
        }
      }
